...
// 2016-05-25 20:53 fractional cascading优化通过了OJ测试，代码很乱，接下来要
// 重构代码并进一步优化性能；代码还是有bug，过不了MOOC的OJ
#include <stdio.h>
#include <stdlib.h>
...
TreeNode *buildRangeTree(TreeNode *parent, Child child, int lo, int hi) {
	if (hi - lo == 1) {
		...
		if (parent != NULL) { // 如果只有一个测站，parent为NULL，这时不需要建立cascade
...
long long report(TreeNode *root, int y1, int y2, int *counter, int lower, int upper) {
	...
	// 处理区间为空的情况
	if (lower == upper && (stations[assoc->yOrder[upper]].y < y1 || stations[assoc->yOrder[upper]].y > y2)) {
		return 0;
	}
	/*
	// 这是个非常隐晦的bug，主要是因为区间的端点是否包含而引起的
	// 程序中的区间有可能为左开右闭，也有可能为两端都闭合
	if (((lo != lower) || hi != (upper + 1)) && lower != upper) {
		if (lower + 1 == lo) {
			//assoc = NULL;
			//printf("%d", assoc->preSum[1]);
			lower = lower + 1;
		}
	}
	*/
	// 处理区间为左开右闭的情况，lower自增一次即可成为两端闭合
	if (stations[assoc->yOrder[lower]].y < y1) {
		lower += 1;
	}
	*counter += upper - lower + 1;
	return assoc->preSum[upper] - assoc->preSum[lower] + stations[assoc->yOrder[lower]].temp;
}
